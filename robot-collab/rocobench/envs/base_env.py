...
class MujocoSimEnv:
    ...
    def __init__(
        ...
        # print(filepath)
        self.xml_file_path = filepath # task.xml filepath
        self.physics = dm_mujoco.Physics.from_xml_path(filepath) # dm_mujoco.mujoco.engine.Physics
        self.home_qpos = home_qpos 
        self.home_keyframe_id = home_keyframe_id

        # try loading home_qpos to the xml file(특정 task에 대한 xml파일 유무확인.) 
        try:
            copy_physics = dm_mujoco.Physics.from_xml_path(filepath) # dm_mujoco.mujoco.engine.Physics
            copy_physics.reset() 
            copy_physics.step()
        ...
        # ['bin', 'table_top', 'apple', 'banana', 'milk', 'soda_can', 'bread', 'cereal']
        ...
        self.save_buffer = deque(maxlen=sim_forward_steps//sim_save_freq) # 저장 데이터 deque
        self.error_threshold = error_threshold
        self.error_freq = error_freq

        # check rendering options(simulation상에 rendering 관련)
    ...
    def reset(self, keyframe_id=0, home_pos=None, reload=False, filepath=None):
        ...
        self.physics.reset(keyframe_id=keyframe_id) #dm_mujoco.mujoco.engine.Physics
        ...
        obs = self.get_obs() # rocobench.envs.task_pack
        self.timestep = 0
        return obs
    ...
    def get_contact(self) -> Dict:
        ...
        # self = task_env
        model = self.model # MjModel
        data = self.data # MjData
        ret = defaultdict(set)

        # contact되어 있는 객체들을 ret에 모음.
        ...
        # also check eq_active(embodiment 부품에 해당하는 객체들도 ret에 모음.)
    ...
    def get_object_states(self, contact_dict) -> List[ObjectState]:
        ...
        # task_objects = ['bin', 'table_top', 'apple', 'banana', 'milk', 'soda_can', 'bread', 'cereal']
    ...
    def save_intermediate_state(self):
        ''' save and return current simulation environment datas'''
        obs = self.get_obs()
        kwargs = dict(
