...
def direct_path(
    ...
    '''
    직관적 path_planning 알고리즘.
    1. qpos1과 qpos2 각각 collision check 후 하나라도 발생 시 None(path없음) return.
    2. qpos1부터 qpos2까지가는 path를 얻음(collision발생 시 제외)
    '''
    if collision_fn(q1) or collision_fn(q2): # q1,q2하나라도 collision 발생 시 None을 반환.
        return None
    
    # q1부터 q2까지 path를 얻음.  [q1 + (q2-q1)*W]  (단, W = [0~1])
    path = [q1]
    for q in extend_fn(q1, q2): # (q2 - q1)*weight 들을 return. <- weight를 바꿔가며 미세하게 다른 target_qpos들을 탐색.
        if collision_fn(q): # 매 point마다 FK을 계산해서, 충돌 발생시 제외.
            return None
        path.append(q)
    
    # q1 ~ q2까지 path를 return.
    return path
...
def birrt(
    ...
    '''
    start qpos -> end qpos까지 path_planning하기
    (1) direct_path_planning: start부터 end까지 direct_path를 계산
        (collision발생 없으면 이것으로 path를 return.)
    (2) RRT기반 path_planning: (1)이 실패하면, RRT를 사용하여 end qpos까지 path_plan수행.
    '''
    if not skip_direct_path:
        start_time = time()

        # 1. 먼저 path를 다이렉트로 구해봄, 만약 path구하는 것에 성공한다면 그 path를 return.
        path = direct_path(start_conf, goal_conf, extend_fn, collision_fn)
        if path is not None:
            return path, f"ReasonDirect_time{time() - start_time}_iter1"

    # 2. 만약 direct_path를 구하는 것에 실패한다면 RRT로 랜덤 샘플링 기반 길찾기 알고리즘 사용.
