...
class LLMResponseParser:
    ...
    def parse(self, obs: EnvState, response: str) -> Tuple[bool, str, List[LLMPathPlan]]: 
        ...
        # 올바른 답변을 수행하지 못했을 경우 feedback text와 함께 False를 return.
        ...
        # check that there is one line per robot(어떤 로봇에 대한 응답인지 알아냄)
    ...
    def add_planned_waypoints(self, ee_target, path_pts, ee_start) -> List[np.ndarray]:
        start_rot = Rotation.from_quat(ee_start[3:]) # rotation matrix로 바꿈.
        end_rot = Rotation.from_quat(ee_target[3:]) # rotation matrix로 바꿈.
        # 두 rotation matrix를 보간한 하나의 rotation mtx
