...
# 6 categories of collaboration tasks
TASK_NAME_MAP = {
...
class LLMRunner:
    def __init__(
        ...
        #(init)Takes in response from LLM query and returns parsed plan for action
        ...
        #(init)Takes in *parsed* LLM response, run task validations,provide feedback if needed
        ...
        if llm_comm_mode in ["plan", "chat"]:
            ...
            #(init)Each round contains multiple prompts, query LLM once per each agent
    ...
    def one_run(self, run_id: int = 0, start_step: int = 0, skip_reset = False, prev_llm_plans = [], prev_response = None, prev_actions = None):
        ...
        self.env.seed(np_seed=run_id) # task_env, rocobench.envs.task_pack
        if not skip_reset:
            self.env.reset(reload=True) # NOTE: need to do this to reset the model.eq_active vals
        env = self.env # rocobench.envs.task_pack
        physics = env.physics # dm_control.mujoco.engine.Physics
        success = False
        ...
        reward = 0 # 초기 reward
        obs = env.get_obs() # task_env.get_obs()

        # 정해진 max_runner_steps 만큼의 step을 반복.
        for step in range(start_step, start_step + self.max_runner_steps):
            ...
            ## ======== add by hscho ========== #
            successful_llm_plan = current_llm_plan
            with open("successful_llm_plan.pkl", "wb") as f:
                pickle.dump(successful_llm_plan, f)
                print("successful_llm_plan dumped in successful_llm_plan.pkl file")
            
            with open("successful_llm_plan.pkl", "rb") as f:
                successful_llm_plan = pickle.load(f)
            current_llm_plan = successful_llm_plan
            ## ======== add by hscho ========== #

            for i, plan in enumerate(current_llm_plan):
                print('tograsp:', plan.tograsp, 'inhand:', plan.inhand, plan.action_strs)

                # llm_plan(subtask)에 대한 path_plan_poilcy를 만듬(qpos등을 waypoint기준으로 업데이트함.)
                ...
                if prev_actions is not None:
                    ...
                    # breakpoint()
                    # (1) 시작qpos부터 목표qpos까지 path-plan, (2)path-plan -> action_signal로 맵핑. 
                    plan_success, reason = policy.plan(env)
                    logging.info(f"Steps: {step} Plan success: {plan_success}, reason: {reason}")
                    if plan_success:
                        logging.info(f"Execute the plan for {len(policy.action_buffer)} steps")

                        plan_fname = os.path.join(step_dir, f"rrt_plan_{i}.pkl")
                        plans = policy.rrt_plan_results # birrt로 path-plan한 결과물
                        with open(plan_fname, "wb") as f:
                            pickle.dump(plans, f)

                        actions_fname = f"{step_dir}/actions_{i}.pkl"
                        with open(actions_fname, "wb") as f:
                            pickle.dump(policy.action_buffer, f) # path-plan을 action으로 변환한 것 + grasping action.
    ...
    def run(self, args):
        ...
        # 기존 trial_number + 1로 현재 trial에 대한 넘버링
...
def main(args):
    ...
    # 수행할 테스크의 task_env 초기화 및 setting.
    # env = rocobench.envs.task_pack
    # env.physics = dm_control.mujoco.engine.Physics
    env = env_cl(
    ...
    # env에 해당하는 테스크를 수행할 로봇 configurations.
    ...
    # init LLMs_agents, runner = <class>LLMRunner 
    runner = LLMRunner(
        env=env,
